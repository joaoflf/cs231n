...
def affine_backward(dout, cache):
    ...
    - db: Gradien =t with respect to b, of shape (M,)
    """
    x, w, b = cache
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xmu = x - mu
        var = 1/N * np.sum(np.square(xmu),axis=0)
        ...
        cache = xmu, var, sqrtvar, ivar, xhat, gamma, beta, eps
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    xmu, var, sqrtvar, ivar, xhat, gamma, beta, eps = cache
    N = dout.shape[0]
    ...
    dvar = dsqrtvar * (1/2) * (1./np.sqrt(var+eps))
    dxmu2 = 1./N * np.ones(dout.shape) * dvar
    dxmu = dxhat*ivar + dxmu2 * 2*xmu
    dmu = -1 * np.sum(dxmu, axis = 0)
    dx = 1./N * np.ones(dout.shape) * dmu + dxmu
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out_width = int(1 + (x.shape[2] + 2 * pad - f_width) / conv_param['stride'])
    out_height = int(1 + (x.shape[3] + 2 * pad - f_height) / conv_param['stride'])
    ...
    for m in range(x.shape[0]):
        ...
        for i in range(out_height):
            ...
            for j in range(out_width):
                ...
                col = x_padded[m,:,i:i+f_height,j:j+f_width].reshape(-1)
                x_col.append(col)
        x_col = np.array(x_col).T

        
        #dot product of input and filter and reshape
        out_m = w_row.dot(x_col) + b.reshape(-1,1)
        out.append(out_m.reshape(f_filters,out_width, out_height))
...
def conv_backward_naive(dout, cache):
    ...
    pad = conv_param['pad']
    f_filters, f_channels, f_width, f_height = w.shape
    dout_n, dout_channels, dout_width, dout_height = dout.shape
    x_n, x_channels, x_width, x_height = x.shape
    
    dw_width = int(1 + (x_width + 2 * pad - dout_width) / conv_param['stride'])
