...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        hidden, hidden_cache = affine_forward(features,W_proj, b_proj)
        
        word_embed, word_embed_cache = word_embedding_forward(captions_in, W_embed) 
        
        if self.cell_type == 'rnn':
            rnn_out, rnn_cache = rnn_forward(word_embed, hidden, Wx, Wh, b)
        else:
            rnn_out, rnn_cache = lstm_forward(word_embed, hidden, Wx, Wh, b)
        ...
        drnn_out, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, scores_cache)
        
        if self.cell_type == 'rnn':
            dword_embed, dhidden, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(drnn_out, rnn_cache)
        else:
            dword_embed, dhidden, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(drnn_out, rnn_cache)
        
        grads['W_embed'] = word_embedding_backward(dword_embed, word_embed_cache)
        
        dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dhidden, hidden_cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
